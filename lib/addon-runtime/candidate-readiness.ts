...
export async function receiveCandidateReadinessV1(input: { body: Buffer; headers: Headers; method: string; pathname: string }) {
  ...
  try {
    ...
    const runtimePrefix =
      request.addonKey === "webshop" ? "WEBSHOP" : "LICENSE_SERVER";
    const runtimeReleaseId = process.env[`${runtimePrefix}_RUNTIME_RELEASE_ID`]?.trim();
    const runtimeBuildId = process.env[`${runtimePrefix}_RUNTIME_BUILD_ID`]?.trim();
    const runtimeArtifactSha256 = process.env[`${runtimePrefix}_RUNTIME_ARTIFACT_SHA256`]?.trim();
    ...
    return signed(auth, 200, { ok: true, buildId: request.buildId, cmsCommitSha, database: "reachable", addons: { [request.addonKey]: { status: "candidate_ready", packageVersion: request.packageVersion, artifactSha256: request.artifactSha256 } } });
  } catch {
