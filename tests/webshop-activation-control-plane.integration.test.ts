...
import {
  deploymentRequestV2Schema,
  deploymentResultV2Schema,
} from "@/lib/addon-runtime/deployment-contract-v2";
...
function controlledStubResult(
  ...
  const noMutationEvidence = {
    ...
    purpose: "addon_deployment_no_mutation" as const,
    operationId: payload.operationId,
    workerJobId,
    targetProfile: "client" as const,
    installationId: payload.installationId,
    installationDeploymentEpoch: payload.installationDeploymentEpoch,
    generation: payload.generation,
    releaseId: payload.releaseId,
    preOperationServingStateHash: payload.preOperationServingStateHash,
    preOperationMigrationLedgerHash: payload.preOperationMigrationLedgerHash,
    cmsControlPlanePhase: "install_pending" as const,
    addonSchemaMutationStarted: false as const,
    serviceMutationStarted: false as const,
    pointerMutationStarted: false as const,
    observedActiveReleaseId: active?.releaseId ?? null,
    observedServicePointerReleaseId: active?.releaseId ?? null,
    lastCompletedWorkerPhase: "accepted" as const,
  };
  return deploymentResultV2Schema.parse({
    ...
    installationId: payload.installationId,
    installationDeploymentEpoch: payload.installationDeploymentEpoch,
    deploymentIntentKey: payload.deploymentIntentKey,
    generation: payload.generation,
    ...
    status: "failed",
    finalPhase: "rejected_before_switch",
    runtimeStatus: active ? "ready" : "not_installed",
    releaseId: payload.releaseId,
    packageName: payload.packageName,
    packageVersion: payload.packageVersion,
    npmTarballSha256: payload.npmTarballSha256,
    npmTarballIntegrity: payload.npmTarballIntegrity,
    artifactSha256: payload.artifactSha256,
    dependencyLockSha256: payload.dependencyLockSha256,
    embeddedManifestSha256: payload.embeddedManifestSha256,
    provenanceSha256: payload.provenanceSha256,
    sbomSha256: payload.sbomSha256,
    publicationAttestationHash: payload.publicationAttestationHash,
    registryPackageVersionId: payload.registryPackageVersionId,
    sourceReleasedAt: payload.sourceReleasedAt,
    publishedAt: payload.publishedAt,
    releaseSigningKid: payload.releaseSigningKid,
    runtimeContractVersion: payload.runtimeContractVersion,
    cmsVersionRange: payload.cmsVersionRange,
    nodeVersionRange: payload.nodeVersionRange,
    nextVersionRange: payload.nextVersionRange,
    minimumCoreSchemaVersion: payload.minimumCoreSchemaVersion,
    schemaVersion: payload.schemaVersion,
    supportedAddonSchemaVersionMin: payload.supportedAddonSchemaVersionMin,
    supportedAddonSchemaVersionMax: payload.supportedAddonSchemaVersionMax,
    migrationBundleHash: payload.migrationBundleHash,
    supportedLicenseEditions: payload.supportedLicenseEditions,
    releaseChannel: payload.releaseChannel,
    entitlementSnapshotHash: payload.entitlementSnapshotHash,
    entitlementLifecycleVersion: payload.entitlementLifecycleVersion,
    entitlementEnvelopeExpiresAt: payload.entitlementEnvelopeExpiresAt,
    activeReleaseId: active?.releaseId ?? null,
    activeArtifactSha256: active?.artifactSha256 ?? null,
    observedServicePointerReleaseId: active?.releaseId ?? null,
    cmsCommitSha: "a".repeat(40),
    observedHostCapabilityDescriptorHash: payload.hostCapabilityDescriptorHash,
    buildId: active?.buildId ?? null,
    migrationLedgerHash: null,
    terminalEvidenceKind: "no_mutation_receipt",
    terminalEvidenceHash: `sha256:${createHash("sha256").update(canonicalJson(noMutationEvidence), "utf8").digest("hex")}`,
    noMutationEvidence,
    errorClass: "retryable",
    errorCode: "controlled_test_stub_no_deployment",
...
beforeEach(async () => {
  ...
  process.env.WEBSHOP_DEPLOYMENT_RESULT_AUTH_SECRET =
    "client-result-test-secret-0123456789";
...
test(
  "CMS lifecycle migration durably fences finalization and receipt evidence",
  ...
  async () => {
    ...
    assert.deepEqual(rows.rows, [
      { state: "lifecycle_finalization_pending", receipt_role: "deactivation" },
    ]);
  },
);

test(
  "PayPal target persists a durable Webshop deployment intent",
  { skip: !databaseUrl, concurrency: false },
  async () => {
    ...
    const accepted = await persistVerifiedWebshopActivation({
      claim,
      signedEntitlement: "compact-jws-v2-paypal-target",
      updatedBy: "test-admin",
    });
    const rows = await client!.query(
      "SELECT target_profile, status FROM cms_addon_deployment_outbox WHERE operation_id = $1",
...
test(
  "authenticated worker callback stores one immutable result and returns duplicate or stale ACK without serving-state writes",
  { skip: !databaseUrl, concurrency: false },
  async () => {
    const { persistVerifiedWebshopActivation } =
      await import("@/data/webshop-addon-control-plane");
    const { receiveDeploymentResultV2 } =
      await import("@/lib/addon-runtime/deployment-result-callback");
    const accepted = await persistVerifiedWebshopActivation({
      claim,
      signedEntitlement: "compact-jws-v2-callback-a",
      updatedBy: "test-admin",
    });
    const workerJobId = "77777777-7777-4777-8777-777777777777";
    await client!.query(
      "UPDATE cms_addon_deployment_outbox SET worker_job_id = $1 WHERE operation_id = $2",
      [workerJobId, accepted.operationId],
    );
    await client!.query(
      "UPDATE cms_addon_installations SET deployment_job_id = $1 WHERE addon_key = 'webshop'",
      [workerJobId],
    );
    const row = await client!.query(
      "SELECT payload FROM cms_addon_deployment_outbox WHERE operation_id = $1",
      [accepted.operationId],
    );
    ...
    const noMutationEvidence = {
      ...
      serviceMutationStarted: false as const,
      pointerMutationStarted: false as const,
      observedActiveReleaseId: null,
      observedServicePointerReleaseId: null,
      lastCompletedWorkerPhase: "accepted" as const,
    };
    const result = {
      version: 2 as const,
      ...
      finalPhase: "rejected_before_switch" as const,
      ...
      observedServicePointerReleaseId: null,
      cmsCommitSha: "a".repeat(40),
      observedHostCapabilityDescriptorHash:
        payload.hostCapabilityDescriptorHash,
      buildId: null,
      migrationLedgerHash: null,
      terminalEvidenceKind: "no_mutation_receipt" as const,
      terminalEvidenceHash: `sha256:${createHash("sha256").update(canonicalJson(noMutationEvidence), "utf8").digest("hex")}`,
      noMutationEvidence,
    ...
    const headers = new Headers(
      signDeployRequest({
        secret: process.env.WEBSHOP_DEPLOYMENT_RESULT_AUTH_SECRET!,
        kid: process.env.WEBSHOP_DEPLOYMENT_RESULT_AUTH_KID!,
        requestId,
        timestamp: String(Math.floor(Date.now() / 1000)),
        method: "POST",
        path: "/api/internal/addons/deployment-results",
        body,
      }),
    );
    const applied = await receiveDeploymentResultV2({
      body,
      headers,
      method: "POST",
      pathname: "/api/internal/addons/deployment-results",
    });
    assert.equal(applied.status, 200);
    assert.match(applied.body.toString("utf8"), /"ack":"applied"/);
    const duplicate = await receiveDeploymentResultV2({
      body,
      headers,
      method: "POST",
      pathname: "/api/internal/addons/deployment-results",
    ...
    const state = await client!.query(
      "SELECT status, runtime_status FROM cms_addon_installations WHERE addon_key = 'webshop'",
    );
    ...
    const ledgers = await client!.query(
      "SELECT count(*)::int AS count FROM cms_addon_deployment_results",
    );
    assert.equal(ledgers.rows[0]?.count, 1);

    const requeued = await persistVerifiedWebshopActivation({
      claim,
      signedEntitlement: "compact-jws-v2-callback-requeue",
      updatedBy: "system",
    });
    assert.equal(requeued.status, "install_pending");
    assert.equal(requeued.reused, false);
    assert.notEqual(requeued.operationId, accepted.operationId);
    const lineage = await client!.query(
      `SELECT id::text, generation, supersedes_operation_id::text, status
    ...
    assert.deepEqual(lineage.rows, [
      ...
      {
        id: requeued.operationId,
        generation: 2,
        supersedes_operation_id: accepted.operationId,
        status: "pending",
      },
    ]);
    const retryOutbox = await client!.query(
      `SELECT generation, payload->>'supersedesOperationId' AS supersedes_operation_id,
            payload->>'operationKey' AS operation_key
       FROM cms_addon_deployment_outbox
      WHERE operation_id=$1`,
      [requeued.operationId],
    );
    assert.deepEqual(retryOutbox.rows, [
      {
        generation: 2,
        supersedes_operation_id: accepted.operationId,
        operation_key: requeued.operationKey,
      },
    ]);
  },
);
...
test(
  "pre-mutation worker exhaustion preserves an exact previously ready runtime while terminalizing the new intent",
  { skip: !databaseUrl, concurrency: false },
  async () => {
    const { persistVerifiedWebshopActivation } =
      await import("@/data/webshop-addon-control-plane");
    ...
    const accepted = await persistVerifiedWebshopActivation({
      claim,
      signedEntitlement: "compact-jws-v2-prior-ready",
      updatedBy: "test-admin",
    });
    ...
    const result = controlledStubResult(
      deploymentRequestV2Schema.parse(row.rows[0]?.payload),
      workerJobId,
    ...
    const response = await receiveDeploymentResultV2({
      body,
      headers,
      method: "POST",
      pathname: "/api/internal/addons/deployment-results",
    ...
    const state = await client!.query(
      "SELECT status, runtime_status, installed_release_id::text, installed_build_id, installed_artifact_sha256 FROM cms_addon_installations WHERE addon_key='webshop'",
    );
    assert.deepEqual(state.rows[0], {
      status: "failed",
      runtime_status: "ready",
      installed_release_id: active.releaseId,
      installed_build_id: active.buildId,
      installed_artifact_sha256: active.artifactSha256,
    });
    const terminal = await client!.query(
      "SELECT kind, final_tuple->>'activeReleaseId' AS active_release_id FROM cms_addon_deployment_terminal_receipts WHERE operation_id=$1",
      [accepted.operationId],
    );
    assert.deepEqual(terminal.rows, [
      { kind: "no_mutation_receipt", active_release_id: active.releaseId },
    ]);
  },
);

test(
  "late historical worker result receives stale_epoch_ignored without changing current desired state",
  { skip: !databaseUrl, concurrency: false },
  async () => {
    const { persistVerifiedWebshopActivation } =
      await import("@/data/webshop-addon-control-plane");
    ...
    const accepted = await persistVerifiedWebshopActivation({
      claim,
      signedEntitlement: "compact-jws-v2-stale-a",
      updatedBy: "test-admin",
    });
    ...
    await client!.query(
      "UPDATE cms_addon_installations SET deployment_job_id = $1, installation_deployment_epoch = 2 WHERE addon_key = 'webshop'",
      [workerJobId],
    );
    const row = await client!.query(
      "SELECT payload FROM cms_addon_deployment_outbox WHERE operation_id = $1",
...
test(
  "verified activation commits entitlement, desired state, operation, and outbox before any dispatch",
  { skip: !databaseUrl, concurrency: false },
  async () => {
    const { persistVerifiedWebshopActivation } =
      await import("@/data/webshop-addon-control-plane");
    ...
    assert.deepEqual(
      operations.rows.map((row) => row.status),
      ["pending"],
    );
...
test(
  "revalidation refreshes an exact ready release without opening another deployment epoch",
  { skip: !databaseUrl, concurrency: false },
  async () => {
    const { persistVerifiedWebshopActivation } =
      await import("@/data/webshop-addon-control-plane");
...
test(
  "legacy cutover terminal never retries its old intent; only fresh host capability opens epoch generation one",
  { skip: !databaseUrl, concurrency: false },
  async () => {
    const { persistVerifiedWebshopActivation } =
      await import("@/data/webshop-addon-control-plane");
    const first = await persistVerifiedWebshopActivation({
      claim,
      signedEntitlement: "compact-jws-v2-legacy-a",
      updatedBy: "test-admin",
    });
    await client!.query(
      "UPDATE cms_addon_operations SET status = 'failed', error_code = 'operator_schema_cutover_required', completed_at = now() WHERE id = $1",
      [first.operationId],
    );
    const sameHost = await persistVerifiedWebshopActivation({
      claim,
      signedEntitlement: "compact-jws-v2-legacy-new-token",
      updatedBy: "test-admin",
    });
    assert.equal(sameHost.operationId, first.operationId);
    assert.equal(sameHost.status, "operator_schema_cutover_required");
    const freshHostClaim = {
      ...claim,
      hostCapabilityDescriptorHash: `sha256:${"c".repeat(64)}`,
    };
    const fresh = await persistVerifiedWebshopActivation({
      claim: freshHostClaim,
      signedEntitlement: "compact-jws-v2-legacy-cutover-complete",
      updatedBy: "test-admin",
    });
    assert.equal(fresh.reused, false);
    const operations = await client!.query(
      "SELECT installation_deployment_epoch::text AS epoch, generation, status FROM cms_addon_operations ORDER BY created_at",
    );
    assert.deepEqual(
      operations.rows.map((row) => [row.epoch, row.generation, row.status]),
      [
        ["1", 1, "failed"],
        ["2", 1, "pending"],
      ],
