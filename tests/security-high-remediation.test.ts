...
test("addon secret purposes have no generic or cross-purpose fallback", () => {
  const paths = [
    ...
    "app/api/cron/webshop-license-server-catalog/route.ts",
  ...
  assert.doesNotMatch(
    combined,
    /process\.env\.(AUTH_SECRET|CLERK_SECRET_KEY|CRON_SECRET)/,
  );
  assert.doesNotMatch(
    combined,
    /development-(?:webshop-buy-link|license-server-buy-link|license-server-runtime|license-server-rate-limit)-secret/,
  );
});
...
test("public schema and migration use encrypted pool-license storage", () => {
  ...
  const migration = resolve(
    root,
    "drizzle/0087_webshop_license_key_encryption.sql",
  );
  ...
  assert.equal(
    ...
    true,
    "the additive expand migration must exist",
  );
  assert.match(
    source("drizzle/0087_webshop_license_key_encryption.sql"),
    /ADD COLUMN "encrypted_license_key"/,
  );
});

test(
  "private pool snapshots retain only encrypted-key references",
  {
    skip: !existsSync(
      resolve(root, ".private/webshop/src/data/webshop-license-keys.ts"),
    ),
  },
  () => {
...
test(
  "invite and bootstrap secrets are not carried in URLs, redirects, or logs",
  {
    skip: !existsSync(
      resolve(root, ".private/license-server/app/admin/actions.ts"),
    ),
  },
  () => {
