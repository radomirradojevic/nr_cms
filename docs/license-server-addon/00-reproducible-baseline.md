...
| --- | --- | --- |
| 1.1 Paket i lifecycle | **Delimično potvrđeno** | Paket je `@nr-cms/license-server@0.1.0`; manifest je potpisan lokalnim release autoritetom i pokriva bundle, provenance, SBOM i prazan migration manifest. Build-time registry, navedena entitlement stanja i `edit_existing_only` postoje. Aktuelni deployment worker, međutim, podržava samo Webshop; License Server install/redeploy nije zatvoren kroz njega. |
