...
const cmsVersion = assertConfiguredCmsVersion(process.env.NR_CMS_VERSION);
const cmsReleaseChannel = releaseChannelForVersion(cmsVersion);
...
const cmsReleaseSha =
  process.env.NR_CMS_RELEASE_SHA?.trim() ||
  process.env.VERCEL_GIT_COMMIT_SHA?.trim() ||
  process.env.GITHUB_SHA?.trim() ||
  "";
if (addons.length > 0 && !/^[a-f0-9]{40}$/.test(cmsReleaseSha)) {
  throw new Error(
    "A 40-character CMS Git commit SHA is required for a managed add-on build.",
  );
}
for (const item of addons) {
  ...
  releaseMetadata[item.addonKey] = await verifyInstalledRelease(
    item,
    releasePublicKeys,
  );
  entries.push(
...
const buildId = sha256(Buffer.from(canonicalJson({
  cms: {
    addonRuntimeContractVersion: CMS_ADDON_RUNTIME_CONTRACT_VERSION,
    commitSha: cmsReleaseSha || null,
    coreSchemaVersion: CMS_CORE_SCHEMA_VERSION,
    version: cmsVersion,
  },
  releases: releaseMetadata,
}), "utf8"));
const output = `/* Generated by scripts/generate-addon-registry.mjs. Do not edit manually. */\nexport const addonLoaders = {\n${entries.join("\n")}\n} as const;\n\nexport type ManagedAddonReleaseMetadata = {\n  artifactSha256: string;\n  migrationBundleHash: string;\n  packageName: string;\n  packageVersion: string;\n  releaseId: string | null;\n  releaseSigningKid: string;\n  runtimeContractVersion: "1";\n  schemaVersion: number;\n};\nexport const addonReleaseMetadata: Partial<Record<"webshop" | "license-server", ManagedAddonReleaseMetadata>> = ${JSON.stringify(releaseMetadata, null, 2)};\nexport const cmsVersion = ${JSON.stringify(cmsVersion)};\nexport const cmsReleaseChannel = ${JSON.stringify(cmsReleaseChannel)} as const;\nexport const cmsCoreSchemaVersion = ${JSON.stringify(CMS_CORE_SCHEMA_VERSION)};\nexport const cmsAddonRuntimeContractVersion = ${JSON.stringify(CMS_ADDON_RUNTIME_CONTRACT_VERSION)} as const;\nexport const cmsReleaseSha = ${JSON.stringify(cmsReleaseSha)};\nexport const managedRuntimeBuildId = ${JSON.stringify(buildId)};\n\nexport type RegisteredAddonKey = keyof typeof addonLoaders;\nexport type AddonLoader = () => Promise<unknown>;\n\nexport function getAddonLoader(addonKey: string): AddonLoader | null {\n  return (addonLoaders[addonKey as RegisteredAddonKey] as AddonLoader | undefined) ?? null;\n}\n`;
...
async function verifyInstalledRelease(expected, publicKeys) {
  ...
  if (
    ...
    return verifyInstalledReleaseV2({
      expected,
      publicKeys,
      packageRoot,
      packageJson,
  ...
  if (
    ...
    !/^[a-f0-9]{64}$/.test(manifest.migrationBundleHash ?? "")
  ) {
    throw new Error(
      `Installed addon release lacks immutable schema metadata: ${expected.addonKey}.`,
    );
  }
  return {
    ...
    releaseSigningKid: manifest.signingKid,
