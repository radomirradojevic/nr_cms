...
const webshopMigrationsDir = path.join(root, ".private", "webshop", "migrations");
const adminPasswordPath = "D:\\nr_runtime\\operator-input\\cms-core-postgres-admin.password";
const expectedHash = process.argv.find((value) => value.startsWith("--expect-hash="))?.slice("--expect-hash=".length);
...
function readDatabaseUrl() {
  ...
  if (!line) fail("DATABASE_URL is required only to locate the local PostgreSQL server.");
  const value = line.slice("DATABASE_URL=".length).trim().replace(/^"|"$/g, "");
  const source = new URL(value);
  if (!['localhost', '127.0.0.1', '::1'].includes(source.hostname)) fail("fixture requires a loopback PostgreSQL DATABASE_URL.");
  return source;
}
...
function readAdminPassword() {
  ...
  if (!password) fail("the protected PostgreSQL administrator password file is empty.");
  return password;
}

function quoteIdentifier(identifier) {
  if (!/^[a-z][a-z0-9_]{0,62}$/.test(identifier)) fail("generated fixture database identifier is invalid.");
  return `"${identifier}"`;
}
...
async function dropDatabase(admin, database) {
  ...
  if (exists.rowCount) fail("temporary fixture cleanup did not remove its database.");
}
...
try {
  ...
  for (const migration of migrations) {
    ...
    for (const [index, statement] of statements.entries()) {
      try {
        ...
        fail(`${migration.id} statement ${index + 1}/${statements.length} failed: ${error instanceof Error ? error.message : "unknown PostgreSQL error"}`);
      }
    }
  }
  ...
  for (const trigger of [
    ...
    if (!receipt.triggerNames.includes(trigger)) fail(`baseline is missing ${trigger}.`);
  }
  if (expectedHash && receipt.fingerprint !== expectedHash) fail("postcondition fingerprint does not match the pinned descriptor.");
  ...
  if (adminConnected) await dropDatabase(admin, database).catch(() => undefined);
