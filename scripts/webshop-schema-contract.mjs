...
const manifestPath = path.join(root, "contracts", "webshop-schema-manifest-v1.json");
...
export function quoteWebshopIdentifier(value) {
  if (!identifier.test(value)) fail("static contract contains an invalid PostgreSQL identifier.");
  return `"${value}"`;
}
...
export function loadWebshopSchemaManifest() {
  ...
  if (
    ...
    !/^[a-f0-9]{64}$/.test(manifest?.postconditionSchemaFingerprintSha256 ?? "") ||
    !/^[a-f0-9]{64}$/.test(manifest?.legacyPublicSchemaFingerprintSha256 ?? "") ||
  ...
  return Object.freeze({ ...manifest, manifestHash: sha256(canonicalJson(manifest)) });
}
